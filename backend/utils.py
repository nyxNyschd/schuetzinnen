...
def get_corpus():
    #long_desc_eng = pd.read_csv('../daten.csv', delimiter=',') # <--- erstmal 1 mal dies für Backend laufen lassen
    long_desc_eng = pd.read_csv(os.getcwd() + '/daten.csv', delimiter=',')  # <--- immer für Frontend wenn utils schon einmal duchgeführt wird
...
if __name__ == '__main__':
    docs = clean_corpus()
    tokens = uniq_tokens(docs)
    outfile1 = open('tokens', 'wb')
    pickle.dump(tokens, outfile1)
    outfile1.close()
