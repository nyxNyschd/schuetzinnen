...
def fuzzy_logic(substring):
    highest_value = 0
    most_relevant_word = ' '
    Ratios = process.extract(substring, CLEANED, limit=10, scorer=fuzz.ratio)
    # print(Ratios)
    for index in range(len(Ratios)):
        if len(Ratios[index][0]) > 2:
            most_relevant_word = Ratios[index][0]
            break
...
def similar_search(similar_words):
    ...
    return main_search(similar_words)[:20]
